...
v1 = client.CoreV1Api()
pv_metadata = client.V1ObjectMeta(name='bad-pv-volume-vol', labels={'type': 'local'})
pvc_metadata = client.V1ObjectMeta(name='bad-pv-claim-vol')
pod_metadata = client.V1ObjectMeta(name='bad-pv-pod')
...
pvcsrc=client.V1PersistentVolumeClaimVolumeSource(claim_name='bad-pv-claim-vol')
vol1 = client.V1Volume(name='bad-pv-storage-vol', persistent_volume_claim=pvcsrc)
volmnt = client.V1VolumeMount(name='bad-pv-storage-vol', mount_path='/hostlogs')
container1 = client.V1Container(name='bad-pv-pod', image='ubuntu:latest', command=["sh", "-c", "sleep 1h"], volume_mounts=[volmnt])
