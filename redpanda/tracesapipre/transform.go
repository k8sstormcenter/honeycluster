...
func doTransform(e transform.WriteEvent, w transform.RecordWriter) error {
	...
	query, err := gojq.Parse("select( .process_kprobe != null and .process_kprobe.process.pod.namespace == \"default\" and ( .process_kprobe.policy_name == \"k8s-api-call\" or .process_kprobe.policy_name == \"enumerate-service-account\" ))| .")
	if err != nil {
		return err
	}
