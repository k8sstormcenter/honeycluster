#!/bin/bash
podname=$(kubectl get pods -n honey -l app=clickhouse -o jsonpath='{.items[0].metadata.name}')
kubectl exec -n honey $podname -- clickhouse-client --multiquery --database=default <<'EOF'
